...
function AddTmpAnalisa($kdgejala, $IP) {
	...
	$qry_sakit = mysqli_query($con,$sql_sakit);
...
if ($RbPilih == "YA") {
	...
	$qry_analisa = mysqli_query($con,$sql_analisa);
	...
	if ($data_cek >= 1) {
		...
		$sql_tmp = "SELECT * FROM tmp_analisa 
					...
					AND noip='$NOIP'";
		$qry_tmp = mysqli_query($con,$sql_tmp);
		while ($data_tmp = mysqli_fetch_array($qry_tmp)) {
			...
			$qry_rsakit = mysqli_query($con,$sql_rsakit);
		...
		$qry_rgejala = mysqli_query($con,$sql_rgejala);
...
if ($RbPilih == "TIDAK") {
	...
	if ($data_cek >= 1) {
		...
		$qry_relasi = mysqli_query($con,$sql_relasi);
